...
class FileCodePointer(
    ...
    def load_target(self) -> object:
        ...
        return _load_target_from_module(
            module, self.attribute, f"at module scope in file {self.python_file}."
        )
...
def _load_target_from_module(module: ModuleType, attribute: str, error_suffix: str) -> object:
    from dagster.core.asset_defs import AssetGroup
    from dagster.core.workspace.autodiscovery import ALL_ASSETS_ATTRIBUTE

    if attribute == ALL_ASSETS_ATTRIBUTE:
        return AssetGroup.from_modules([module])
    else:
        if not hasattr(module, attribute):
            raise DagsterInvariantViolationError(f"{attribute} not found {error_suffix}")
